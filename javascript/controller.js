...
//Load Page from url, 404 Not Found redirect to home
function nwaLoadPageFromUrl() {
	var nwaLocationPathname = window.location.pathname;
	for (i in $nwaScope) {
    	for (j in $nwaScope[i]) {
			if ($nwaScope[i][j].nwaPageHistoryUrl == nwaLocationPathname) {nwaPageFetch(false,i,j); return;}
    	}
	}
	for (i in $nwaScope) {
		for (j in $nwaScope[i]) {
			if ($nwaScope[i][j].nwaPageHistoryUrl != '/' && nwaLocationPathname.startsWith($nwaScope[i][j].nwaPageHistoryUrl+'/')) {
				var id = nwaLocationPathname.replace($nwaScope[i][j].nwaPageHistoryUrl+'/', '');
				nwaPageFetch(false,i,j,id); return;
			}
		}
	}
...
function nwaPageFetch(nwaHistoryPushState, controller, action, id) {
    ...
    //template parts
    if ($nwaScope[controller][action].nwaPageFetch.header !== false)
    	nwaSelect('body').classList.add('nwaHeader');
    else
    	nwaSelect('body').classList.remove('nwaHeader');
    if ($nwaScope[controller][action].nwaPageFetch.footer === true)
    	nwaSelect('body').classList.add('nwaFooter');
    else
    	nwaSelect('body').classList.remove('nwaFooter');
    if ($nwaScope[controller][action].nwaPageFetch.nav === true)
    	nwaSelect('body').classList.add('nwaNav');
    else
    	nwaSelect('body').classList.remove('nwaNav');

	nwaAutoToggleNav();

    //Change page name on header before page loads
    if ($nwaScope[controller][action].nwaPageFetch.header !== false) {
        nwaSelect('header h1').innerHTML = $nwaScope[controller][action].nwaPageFetch.header;
    }
	...
	if (nwaHistoryPushState === true)
    	window.history.pushState({}, '', nwaPageHistoryUrl);
    else
    	window.history.replaceState({}, '', nwaPageHistoryUrl);
    ...
    .then((html) => {
        ...
        if ($nwaScope[controller][action].nwaPageData) {
        	...
        	if (id) url += '/'+id;
        	nwaAjax({method:'GET', url:url, success: function(status,responseText) {
        		//Run page data on load function from user app.js file
            	$nwaScope[controller][action].nwaPageData.onResponse(status,responseText);
...
function nwaPageFormSubmitAjaxEventMaker() {
    nwaSelect('main form').addEventListener('submit', function(e){
        ...
        console.groupCollapsed('Crj form: ' + nwaSelect('main').getAttribute('controller') + ' ' + nwaSelect('main').getAttribute('action') + ' ' + nwaSelect('main form').id);

        var data = {};
        for (var i=0; i<nwaSelect('main form').length; i++) {
            if (nwaSelect('main form').elements[i].getAttribute("name"))
                data[nwaSelect('main form').elements[i].getAttribute("name")] = nwaSelect('main form').elements[i].value;
        }
        var url = $nwaScope[nwaSelect('main').getAttribute('controller')][nwaSelect('main').getAttribute('action')].nwaPageForm.url;
        nwaAjax({reference:nwaSelect('main form'), method:'POST', url:url, data:data, success: function(status,responseText,formId) {
            console.groupEnd();
            $nwaScope[nwaSelect('main').getAttribute('controller')][nwaSelect('main').getAttribute('action')].nwaPageForm.onResponse(status,responseText);
...
function nwaAjax(object) {
    ...
    console.log('Client '+object.method+' '+object.url+' : '+JSON.stringify(object.data));

    nwaHttp.onreadystatechange = function() {
        if (this.readyState == 4) {
            console.groupCollapsed('Server ' + this.status + ': ');
            ...
            if (this.status < 300 && this.status != 0) {
                if (typeof object.success === 'function')
                    if (typeof object.reference !== 'undefined')
                        ...
                        object.success(this.status, this.responseText);
            } else {
                if (typeof object.fail === 'function')
                    if (typeof object.reference !== 'undefined')
                        object.fail(this.status, this.responseText, object.reference.id);
                    else
                        object.fail(this.status, this.responseText);
                if (this.status == 0)
                    alert('server status: 0');
                else
                    if (confirm('Server error ' + this.status + ': ' + this.responseText + '\n' + 'Reload the app?')) {
